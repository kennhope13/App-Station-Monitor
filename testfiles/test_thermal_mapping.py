...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* Lớp dưới: iframe stream — pointer-events:none để click xuyên lên overlay */
        .stream-iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none; z-index: 1; pointer-events: none; }

        /* Lớp trên: overlay trong suốt để vẽ marker/ROI */
...
<body>
    ...
    <div class="main-wrapper">
        <div class="video-panel">
            ...
            <div class="video-container" id="container-vis">
                <!-- Lớp dưới: go2rtc stream qua video-stream.js — tự xử lý WebRTC/MSE/H.265 -->
                <iframe id="iframe-vis" class="stream-iframe"></iframe>
                <!-- Lớp trên: overlay trong suốt — click/drag để đặt điểm và vẽ ROI -->
        ...
        <div class="video-panel">
            ...
            <div class="video-container therm" id="container-therm">
                <iframe id="iframe-therm" class="stream-iframe"></iframe>
    ...
    // ── Cập nhật iframe stream — go2rtc tự xử lý WebRTC/MSE/H.265 ──
    function updateStreams() {
        const vis   = document.getElementById('go2rtc-vis').value.trim();
        const therm = document.getElementById('go2rtc-therm').value.trim();
        document.getElementById('iframe-vis').src   = `/camera-stream?src=${encodeURIComponent(vis)}`;
        document.getElementById('iframe-therm').src = `/camera-stream?src=${encodeURIComponent(therm)}`;
        showToast("Đã cập nhật stream!");
    }
    updateStreams();
    ...
    function createRoiElement(id, name) {
        ...
        // Dùng span riêng cho giá trị nhiệt → cập nhật textContent, không parse HTML
        lbl.innerHTML = `<b>${name}</b> | Max: <span class="roi-max" style="color:#00ff00">--°C</span>`;
        box.appendChild(lbl);
...
'...';
    ...
    // Cache getBoundingClientRect — tránh layout reflow mỗi mousemove
    var _cachedRects = {};
    function getContainerRect(key, el) {
        if (!_cachedRects[key]) _cachedRects[key] = el.getBoundingClientRect();
        return _cachedRects[key];
    }
    window.addEventListener('resize', () => { _cachedRects = {}; });

    document.addEventListener('mousemove', function(e) {
        if (!isDragging || !dragRef || !mapping) return;
        var container = dragSrc === 'vis' ? containerVis : containerTherm;
        var rect = getContainerRect(dragSrc, container);
    ...
    // Poll nhiệt độ: dùng setTimeout chained thay setInterval
    // → request tiếp chỉ gửi sau khi request trước xong, không chồng nhau
    var _tempPollActive = false;
    async function pollTemps() {
        if (!activeMarkers.length && !activeRois.length) {
            setTimeout(pollTemps, 150);
            return;
        }
        ...
        try {
            const response = await fetch('/get_temps', {
                ...
                body: JSON.stringify({ points, rois })
            });
            ...
            if (data.points) {
                data.points.forEach(res => {
                    ...
                    if (m && !isDragging && res.temp !== null) {
                        ...
                        // textContent thay innerHTML — không tạo DOM mới
                        [m.visEl, m.thermEl].forEach(el => {
                            el.tempEl.textContent = str;
                            el.tempEl.style.color = color;
            ...
            if (data.rois) {
                data.rois.forEach(res => {
                    ...
                    if (r && res.max !== null) {
                        ...
                        // Dùng textContent trên các span con thay vì innerHTML
                        r.visEl.lbl.querySelector('.roi-max').textContent = res.max.toFixed(1) + '°C';
                        r.visEl.lbl.querySelector('.roi-max').style.color = color;
                        r.thermEl.lbl.querySelector('.roi-max').textContent = res.max.toFixed(1) + '°C';
                        r.thermEl.lbl.querySelector('.roi-max').style.color = color;
                    }
                });
            }
        } catch(e) {}

        setTimeout(pollTemps, 100);
    }
    pollTemps();
...
# Serve static JS từ frontend/public (dùng chung với React app)
FRONTEND_PUBLIC = '/home/admin-/stationos-main/frontend/public'

@app.route('/video-stream.js')
def serve_video_stream_js():
    with open(f'{FRONTEND_PUBLIC}/video-stream.js', 'r', encoding='utf-8') as f:
        return f.read(), 200, {'Content-Type': 'application/javascript'}

@app.route('/video-rtc.js')
def serve_video_rtc_js():
    with open(f'{FRONTEND_PUBLIC}/video-rtc.js', 'r', encoding='utf-8') as f:
        return f.read(), 200, {'Content-Type': 'application/javascript'}

@app.route('/camera-stream')
def serve_camera_stream():
    """Serve camera-stream.html với go2rtc trỏ về localhost:1984."""
...
el.src = new URL('api/ws?src={src}', 'http://localhost:1984');
...
def main():
    ...
    app.run(host='0.0.0.0', port=8999, debug=False, use_reloader=False, threaded=True)
