...
# Apply the current masking in pd_region_analyzer.py
...
mask[int(small_h * 0.35):int(small_h * 0.55), int(small_w * 0.70):int(small_w * 0.90)] = 0
...
for i, c in enumerate(contours):
    ...
    print(f"  Contour {i}: area={area:.1f}, bbox=({x},{y},{cw},{ch}) (Norm X: {x/small_w:.3f} to {(x+cw)/small_w:.3f}, Norm Y: {y/small_h:.3f} to {(y+ch)/small_h:.3f})")
