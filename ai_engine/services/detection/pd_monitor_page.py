"""
pd_monitor_page.py — HTML page cho PD boundary monitor (1:1 Port từ test_cam153_boundaries.py)
"""
import re

def get_pd_monitor_html(device_id: str, device_name: str, camera_ip: str,
                         stream_id: str, ai_engine_url: str,
                         token: str = "", backend_url: str = "http://localhost:5000",
                         theme: str = "dark") -> str:
    
    go2rtc_base = re.sub(r':\d+$', ':1984', backend_url)
  ...
  <div class="video-panel">
    <div class="video-wrap" id="wrap">
      <div class="cam-label">
        ...
        {device_name} | {camera_ip}
      </div>
      <!-- Dùng luồng WebRTC mượt mà trực tiếp từ go2rtc -->
      <iframe id="stream" src="{go2rtc_base}/webrtc.html?src={stream_id}" allow="autoplay" style="border: none; width: 100%; height: 100vh; pointer-events: none; display: block;"></iframe>
...
function render() {{
    ...
    boundaries.forEach((b, i) => {{
        ...
        let poly = [];
        if (b.vertices) {{
            poly = b.vertices.map(v => [v.x / 100, v.y / 100]);
        }} else if (b.polygon) {{
            poly = typeof b.polygon === 'string' ? JSON.parse(b.polygon) : b.polygon;
        }}
        
        const pts = poly.map(p => `${{p[0]*1000}},${{p[1]*1000}}`).join(' ');
        ...
        if (poly.length > 0) {{
            const xs = poly.map(p => p[0]*1000);
            const ys = poly.map(p => p[1]*1000);
            ...
            const pos = b.namePosition || b.labelPos || 'bottom';
            if (pos === 'top') {{
...
async function poll() {{
    try {{
        const r = await fetch(`${{AI_URL}}/pd-monitor/${{DEVICE_ID}}/state?token=${{TOKEN}}&backend=${{BACKEND}}&_=${{Date.now()}}`);
...
async function delBoundary(i) {{
    ...
    await fetch(`${{AI_URL}}/pd-monitor/boundary/${{b.id}}?token=${{TOKEN}}&backend=${{BACKEND}}`, {{method: 'DELETE'}});
...
function editBoundary(i) {{
    ...
    document.getElementById('posInput').value = b.namePosition || b.labelPos || 'bottom';
...
async function saveBoundary() {{
    ...
    let url = `${{AI_URL}}/pd-monitor/${{DEVICE_ID}}/boundary?token=${{TOKEN}}&backend=${{BACKEND}}`;
    let method = 'POST';
    if (editingIdx !== null) {{
        url = `${{AI_URL}}/pd-monitor/boundary/${{lastState.boundaries[editingIdx].id}}?token=${{TOKEN}}&backend=${{BACKEND}}`;
        method = 'PUT';
    }}
    
    const body = {{
        name: name, Name: name,
        polygon: JSON.stringify(pendingPolygon), Polygon: JSON.stringify(pendingPolygon),
        thresholds: JSON.stringify({{namePosition: pos}}), Thresholds: JSON.stringify({{namePosition: pos}}),
        type: 'pd', Type: 'pd',
        severityLevel: 'alarm', SeverityLevel: 'alarm',
