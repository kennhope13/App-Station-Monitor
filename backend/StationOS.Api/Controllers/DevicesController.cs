...
public class DevicesController : ControllerBase
{
    ...
    /// - Nếu SubType == "pd"  → tạo 1 thiết bị camera_pd  (kênh 101)
    /// - Nếu HasThermal      → tạo 1 thiết bị camera_dual (kênh 101 + 201)
    /// - Còn lại             → tạo 1 thiết bị camera_cctv (kênh 101)
    ...
    public async Task<IActionResult> Create([FromBody] CreateDeviceRequest req)
    {
        ...
        if (req.Type.StartsWith("camera"))
        {
            ...
            var ip       = cfg.TryGetValue("ip", out var ipVal) ? ipVal?.ToString() : null;
            var username = cfg.TryGetValue("username", out var uVal) ? uVal?.ToString() : "admin";
            var password = cfg.TryGetValue("password", out var pVal) ? pVal?.ToString() : "";
        ...
        if (device.Type == "camera_pd")
        {
            ...
            var ip2   = cfg2.TryGetValue("ip", out var i2Val) ? i2Val?.ToString() : null;
            var u2    = cfg2.TryGetValue("username", out var u2Val) ? u2Val?.ToString() : "admin";
            var p2    = cfg2.TryGetValue("password", out var p2Val) ? p2Val?.ToString() : "";
            if (string.IsNullOrEmpty(u2)) u2 = "admin";
            if (p2 == null) p2 = "";
    ...
    /// <summary>
    /// Trả về credentials (username + password) đã giải mã — chỉ dùng trong UI admin.
    /// </summary>
    ...
    public async Task<IActionResult> GetCredentials(Guid id)
    {
        var d = await _db.Devices.FindAsync(id);
        if (d == null) return NotFound();
        var plain = _crypto.DecryptPasswordInConfigJson(d.Config);
        var cfg = TryParseConfig(plain);
        return Ok(new {
            username = cfg.GetValueOrDefault("username")?.ToString() ?? "",
            password = cfg.GetValueOrDefault("password")?.ToString() ?? "",
        });
    }
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateDeviceRequest req)
    {
        ...
        // QUAN TRỌNG: phải giải mã config trước khi gửi go2rtc (vì DB lưu mã hóa)
    ...
    /// <summary>
    /// Debug endpoint để truy vấn camera 192.168.10.120 trực tiếp từ luồng mạng Host
    /// </summary>
    [HttpGet("devices/debug-thermal-120")]
    [AllowAnonymous]
    public async Task<IActionResult> DebugThermal120()
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("--- HIKVISION 192.168.10.120 HOST DIAGNOSTICS REPORT ---");
        sb.AppendLine($"Time: {DateTime.UtcNow}");

        string ip = "192.168.10.120";
        string user = "admin";
        string password = "Demo@2024";

        var endpoints = new[]
        {
            ("/ISAPI/System/deviceInfo", "Device Info"),
            ("/ISAPI/Thermal/channels", "Thermal Channels"),
            ("/ISAPI/Thermal/channels/1/thermometry/rules", "Rules Ch1"),
            ("/ISAPI/Thermal/channels/2/thermometry/rules", "Rules Ch2"),
            ("/ISAPI/Thermal/channels/1/thermometry/rulesList", "RulesList Ch1"),
            ("/ISAPI/Thermal/channels/2/thermometry/rulesList", "RulesList Ch2")
        };

        foreach (var (path, label) in endpoints)
        {
            sb.AppendLine($"\n[+] Probing {label} ({path})...");
            try
            {
                ...
                using var res = await client.GetAsync($"http://{ip}{path}");
                
                sb.AppendLine($"    Status Code: {res.StatusCode}");
                if (res.IsSuccessStatusCode)
                {
                    var text = await res.Content.ReadAsStringAsync();
                    sb.AppendLine("    [Body XML/JSON]:");
                    sb.AppendLine(text.Length > 2000 ? text.Substring(0, 2000) + "\n    [...truncated...]" : text);
                }
                else
                {
                    var errText = await res.Content.ReadAsStringAsync();
                    sb.AppendLine($"    Error details: {errText}");
                }
            }
            catch (Exception ex)
            {
                sb.AppendLine($"    Exception error: {ex.Message}");
            }
        }

        var report = sb.ToString();
        var outputPath = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), "camera_check_output.txt");
        await System.IO.File.WriteAllTextAsync(outputPath, report);
        
        return Ok(new { message = "Probe completed successfully!", filePath = outputPath, report = report });
    }
}
